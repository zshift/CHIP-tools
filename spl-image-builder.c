 ...
 */
static int decode_bch(struct bch_control *bch,
...
static int create_image(const struct image_info *info)
{
	...
	int pagepadsize = info->page_size - info->usable_page_size;
	...
	if (usable_page_size < info->page_size + info->oob_size) {
		fread(buffer + usable_page_size, 1,
		      info->page_size + info->oob_size - usable_page_size,
		      rnd);
	...
	while (1) {
		...
		if (srcpadsize)
			fread(data + cnt, 1, srcpadsize, rnd);
